...
package:
	...
	sed -e "s/__RELEASE__/$(shell git rev-list HEAD --count)/g" support/pingtest.spec > ${TMPDIR}/pingtest.spec
	rm -rf pkg || :
	mkdir pkg

	cp ./pingtest ${TEMPDIR}/SOURCES

	rpmbuild -vv -bb --target="noarch" --clean --define "_topdir ${TEMPDIR}" ${TMPDIR}/pingtest.spec
